/**
 * AI 问答服务 — 基于 LLM + SQLite RAG，支持多轮对话记忆
 */
...
7. 回答使用中文
8. 你支持多轮对话——用户可能会追问或引用之前的对话内容，请保持上下文连贯`
...
/**
 * 流式 AI 回答（支持多轮对话历史）
 * @param history 之前的对话历史（最多保留最近 5 轮 = 10 条消息）
 */
export async function* streamAIResponse(
  ...
  // 构建消息列表：system + 历史对话（最近 5 轮） + 当前消息
  const MAX_HISTORY = 10 // 5 轮 * 2（user+assistant）
